...
function init() {//主函数
	var date = new Date();
	UID = document.getElementsByName("studentno").value;
	//UID = "MoRen";
...
function mouseDown(event) {
	...
	var behInfo = "";
	if (e.button == 0) {
		behInfo = "mousedown_0\t" + x + "\t" + y + "\t" + new Date().valueOf() + enter;
	} else if (e.button == 1) {
		behInfo = "mousedown_1\t" + x + "\t" + y + "\t" + new Date().valueOf() + enter;
	} else {
		behInfo = "mousedown_2\t" + x + "\t" + y + "\t" + new Date().valueOf() + enter;
	}
	collect(behInfo, "pc");
}
...
function collect(behInfo, bType,fpath) {//异步提交行为信息
	var xmlhttp;
...
//			document.getElementById("test").innerHTML = xmlhttp.responseText;http://202.117.3.78:8080
	...
	xmlhttp.open("POST", "http://192.168.60.80:8080/WebDemoTest/AJAXServlet", true);
